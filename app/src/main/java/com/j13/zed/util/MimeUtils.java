...
public final class MimeUtils {
    ...
    static {
        ...
        add("audio/x-monkeys-audio", "ape");
        add("audio/x-pn-realaudio", "ra");
        add("video/x-pn-realvideo", "rm");
        add("video/x-pn-realvideo", "rmvb");
...
//        add("image/tiff", "tif");
        ...
        add("application/octet-stream", "vob");
        add("video/MP2T", "ts");
        add("video/mp4", "f4v");
     ...
     * Get mimetype from extension firstly. If the type is null, get mimetype from media database.
     * Because, get it from database may be wrong, eg: some apks, its mimetype is video.
     *
    ...
    public static String guessMimeTypeFromExtension(String extension) {
        ...
        if (!TextUtils.isEmpty(extension)) {
            mimeType = extensionToMimeTypeMap.get(extension.toLowerCase());
        }
        return TextUtils.isEmpty(mimeType) ? MimeUtils.MIME_ALL : mimeType;
    }

    public static int guessFileTypeFromExtension(String extension) {
        String mimeType = MimeUtils.guessMimeTypeFromExtension(extension);
        return guessFileTypeFromMimeType(extension, mimeType);
    }

    public static int guessFileTypeFromMimeType(String extension, String mimeType) {
        if ((mimeType != null && mimeType.startsWith(MimeUtils.MIME_TYPE_AUDIO))
                || (!TextUtils.isEmpty(extension) && MimeUtils.EXTRA_AUDIO_EXTENSION.contains(extension.toLowerCase()))) {
            return MEDIA_TYPE_AUDIO;
        } else if (mimeType != null && mimeType.startsWith(MimeUtils.MIME_TYPE_VIDEO)) {
            return MEDIA_TYPE_VIDEO;
        } else if (mimeType != null && mimeType.startsWith(MimeUtils.MIME_TYPE_IMAGE)) {
            return MEDIA_TYPE_IMAGE;
        } else if (mimeType != null && mimeType.equals(MIME_APK)) {
    ...
    public static String getMimeTypeFromMediaDatabase(Context context, String filePath) {
        ...
        return TextUtils.isEmpty(type) ? MimeUtils.MIME_ALL : type;
    }
    ...
    public static boolean isImage(Context context, String filePath) {
        String type = MimeUtils.guessMimeTypeFromExtension(getFileExt(filePath));
        if (type.equals(MimeUtils.MIME_ALL)) {
            type = MimeUtils.getMimeTypeFromMediaDatabase(context, filePath);
        }
        return type.startsWith(MimeUtils.MIME_TYPE_IMAGE);
    }
